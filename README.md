...
│   ├── src/app/(app)/       # Authenticated pages (discover, onboarding)
│   ├── src/app/login/       # SSO login page
│   ├── src/app/signout/     # Sign-out confirmation
│   ├── src/app/api/auth/    # NextAuth API routes
│   ├── src/components/      # Sidebar, modals, UI components
│   ├── src/lib/             # MCP client, profile, PDF extract, keys
...
A modern web interface for the job search bot. Runs locally or deployed to Vercel.

### Features

- **SSO Authentication** — Google and LinkedIn sign-in via NextAuth v5
- **PDF Resume Upload** — Drag-and-drop with pdfjs-dist text extraction + Tesseract.js OCR fallback for scanned documents
- **AI Profile Extraction** — Gemini analyses your resume and extracts skills, seniority, search terms (8-model cascade)
- **Split-pane Discover** — Job list on left, full detail on right. Keyboard navigation (j/k/Esc)
- **Auto-score Top 5** — After search, top results are scored automatically against your resume
- **One-click Cover Letter** — Gemini generates tailored resume bullets + cover letter per job
- **Filters** — Country, job type, freshness, score threshold, source, visa sponsorship
- **Company Search** — Search 30+ companies directly via Greenhouse/Lever APIs (free, no key needed)
- **Profile Persistence** — Saved in localStorage, no re-onboarding on refresh
...
cd web
npm install
cp .env.example .env.local   # Fill in OAuth credentials + AUTH_SECRET
...
| `NEXT_PUBLIC_MCP_URL` | MCP server URL (default: `http://localhost:8000/mcp`) |
